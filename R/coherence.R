...
virtualDocuments <- function(itoks, window = 10) {
  ...
  if (!is.numeric(window)) {
    stop(
      "Incorrect window argument. Please enter a valid number or the character \"boolean\"."
    )
  }
  if (any(quanteda::ntoken(vD) < window)) {
    warning(
      "Some documents are shorter than the sliding window. Boolean count will be applied for those document instead."
    )
    short <- vD[quanteda::ntoken(vD) < window]
    vD <- vD[quanteda::ntoken(vD) >= window]
  }
  vD <- quanteda::tokens_chunk(
    vD,
    size = window,
    overlap = window - 1,
    use_docvars = FALSE
  ...
  if (exists("short", inherits = FALSE)) {
    vD <- vD + short
  }
  vD
}
...
C_NPMI <- function(
  ...
  if (is.null(NPMIs)) {
    NPMIs <- computeNPMI(x$tokens, window)
  }

  coherence <- matrix(0, length(topics), length(sentiments))
  dimnames(coherence) <- list(
    paste0("topic", topics),
    paste0("sentiment", sentiments)
  )
  ...
  for (t in 1:length(topics)) {
    for (s in 1:length(sentiments)) {
      ...
      top <- merge(
        top[L1 == topics[t] & L2 == sentiments[s]],
        x$vocabulary,
        by.x = "word",
        by.y = "word"
      )
...
C_V <- function(
  ...
  tops <- merge(
    top_words_dt(x, nWords, top_method),
    x$vocabulary,
    by.x = "word",
    by.y = "word",
    sort = FALSE
