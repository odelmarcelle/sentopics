generateVocab <- function(
  ...
  for (i in 1:nTopics) {
    ...
    for (j in 1:nSentiments) {
      ...
      topicVocab[(1:nWords) + (nWords * (j - 1))] <- paste0(
        topicVocab[(1:nWords) + (nWords * (j - 1))],
        "sent",
        j,
        "w",
        1:nWords
  ...
  if (nCommonWords > 0) {
    topicVocab <- as.vector(sapply(1:nTopics, function(x) {
      paste0("topic", x, "w", 1:nCommonWords)
    }))

    vocab <- c(vocab, topicVocab)

    topicVocab <- as.vector(sapply(1:nSentiments, function(x) {
      paste0("sentiment", x, "w", 1:nCommonWords)
    }))
    vocab <- c(vocab, topicVocab)
  }
  ...
  for (i in 1:nTopics) {
    for (j in 1:nSentiments) {
      ...
      if (FLAG) {
        betaa[
          c(
            ((i - 1) * nWords * nSentiments) +
              ((j - 1) * nWords + 1):(j * nWords),
            seq(
              nTopics * nSentiments * nWords + 1 + (i - 1) * nCommonWords,
              length.out = nCommonWords
            ),
            seq(
              nTopics *
                nSentiments *
                nWords +
                nTopics * nCommonWords +
                1 +
                (j - 1) * nCommonWords,
              length.out = nCommonWords
            )
          ...
          i,
          j
        ] <- rep(1 / (nWords + nCommonWords * 2), nWords + nCommonWords * 2)
      } else {
        betaa[
          c(
            ((i - 1) * nWords * nSentiments) +
          ...
          i,
          j
        ] <- rdirichlet(1, rep(betaDirichlet, nWords + nCommonWords * 2))
      }
    }
  ...
  dimnames(betaa) <- list(
    word = vocab,
    topics = paste0("topic", 1:nTopics),
    sent = paste0("sent", 1:nSentiments)
  )
  colSums(betaa)
...
generateDocuments <- function(
  ...
  if (length(L1prior) == 1) {
    ...
    stop(
      "Please provide a valid L1prior: either an unique value, an unique value per class, or a complete matrix T x C"
    )
  }
  ...
  if (length(L2prior) == 1) {
    ...
    stop(
      "Please provide a valid L2prior: either an unique value, an unique value per class, or a complete array T x S x C"
    )
  }
  ...
  for (i in 1:nDocs) {
    ...
    pi <- t(sapply(1:dim(vocab)[3], function(x) {
      rdirichlet(1, L2priorArray[x, , c])
    }))
...
generatePartialLexicon <- function(toks, Sindex = c(1, 3)) {
  ...
  lex <- vocab[grep(
    paste0("sentiment[", paste0(Sindex, collapse = ""), "]"),
    word
  )]
  lex$value <- 1
  lex[grep("sentiment1", word), "value" := -1][]
  if (2 %in% Sindex & 3 %in% Sindex) {
    lex[grep("sentiment2", word), "value" := 0][]
  }

  quanteda::dictionary(list(
    "negative" = lex[value == -1, word],
    "positive" = lex[value == 1, word]
  ))
}


distToGenerative <- function(
  x,
  ...
  if (!inherits(x, "multi_chains")) {
    stop("Only valid for multi_chains objects.")
  }
  if (nrow(x[[1]]$vocabulary) < nrow(generativeVocabulary)) {
    stop(
      "Not all words of the generative vocabulary are present in the generated text."
    )
  }

  x[[length(x) + 1]] <- x[[1]]
  x[[length(x)]]$phi <- generativeVocabulary[
    order(match(rownames(generativeVocabulary), x$vocabulary$word)),
    ,
    ,
    drop = FALSE
  ]
  ...
  if (plot) {
    ...
    plot(
      ...
      xlab = "Coordinate 1",
      ylab = "Coordinate 2",
      main = paste0("Distance to true vocabulary")
    )
    graphics::text(
      coord[-attr(x, "nChains"), 1],
      coord[-attr(x, "nChains"), 2],
      rownames(coord),
      cex = 0.8
    )
    ...
    graphics::abline(
      v = coord[attr(x, "nChains"), 1],
      h = coord[attr(x, "nChains"), 2],
      col = "gray75"
    )
  } else {
    if (full) {
