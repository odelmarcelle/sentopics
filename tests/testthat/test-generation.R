...
vocab2 <- vocabFromList(unname(list(
  ...
  topic2 = list(
    sent1 = c("surprising", "amazing"),
    sent2 = c("boring", "annoying")
  ),
...
test_that("generateDocuments incorrect priors", {
  ...
  expect_error(
    corpus <- generateDocuments(
      vocab,
      nClass = 2,
      L1prior = matrix(1:6, 3, 2),
      L2prior = array(1:12, dim = c(2, 3, 2)),
      nDocs = 20
    ),
  ...
  expect_error(
    corpus <- generateDocuments(
      vocab,
      nClass = 2,
      L1prior = matrix(1:4, 2, 2),
      L2prior = array(1:18, dim = c(2, 3, 3)),
      nDocs = 20
    ),
