...
ECB_press_conferences <- corpus(ECB_press_conferences, docid_field = "paragraph_id", text_field = "content")
# 
...
attr(ECB_press_conferences, "docvars")$docid_ <- factor(ECB_press_conferences$doc_id, levels = unique(ECB_press_conferences$doc_id))
...
{
  n <- ntoken(ECB_press_conferences, remove_punct = TRUE,
...

{
  ...
  (parsed <- spacyr::spacy_parse(ECB_press_conferences_tokens,
                                 entity = FALSE, nounphrase = FALSE)) |>
    system.time() |>
    print()
  toks <- quanteda::as.tokens(subset(parsed, pos %in% c("NOUN", "ADJ", "PROPN")), use_lemma = TRUE)
  ECB_press_conferences_tokens <- ECB_press_conferences_tokens[names(ECB_press_conferences_tokens) %in% names(toks)]
  {
...
## Remove set of stopwords
ECB_press_conferences_tokens <- tokens_remove(ECB_press_conferences_tokens, c(stopwords("en", source = "marimo"), "lady","term", "long", "medium", "short", "gentleman", "speech", "see", "ladies", "gentlemen", "year", "years", "also", "can", "may", "new", "one", "time", "important", "level", "however", "need", "first", "even", "let", "like", "two", "three", "four", "five", "framework", "second", "third", "now", "well", "since", "recent", "process", "current", "large", "many", "future", "low", "still", "way", "high", "across", "key", "today", "last", "main", "different", "much", "take", "make", "higher", "lower", "made", "towards", "within", "period", "view", "past", "present", "already", "use", "ensure", "fact", "analysis", "number", "numbers", "objective", "part"))
## Detect bigrams
quanteda.textstats::textstat_collocations(ECB_press_conferences_tokens, min_count = 500, size = 2)
ECB_press_conferences_tokens <- tokens_compound(
  ECB_press_conferences_tokens,
  sapply(quanteda.textstats::textstat_collocations(ECB_press_conferences_tokens, min_count = 500, size = 2)$collocation, phrase)
)
...
ECB_press_conferences_tokens <- as.tokens(dfm2, tokens = ECB_press_conferences_tokens, padding = TRUE, ignore_list = unlist(LoughranMcDonald, use.names = FALSE))
